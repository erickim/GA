...
The Genetic algorithms (GAs) are stochastic search algorithms that mimic the process of Darwinian natural selection. GAs simulate the biological evolution, where breeding among highly fit organisms that ensures desirable attributes to be passed to future generations, thereby provides a set of increasingly good candidate solutions to the optimization. 
...
Selection mechanism mimic the process by which parents are chosen to produce offspring. Crossover and mutation operator is used to produce offspring chromosomes from chosen parent chromosomes.
...
The main function select() will initialize the first generation by calling initialize(), then select the individuals that serve as parents for the second generation by calling selection(). Then the parents produce the two individuals of the second generation by calling corssover() and finally generate mutation on the offsprings by calling mutate(). By then, the second generation is completely generated. Then we do iteration over the whole procedure above except for the initialization, because we treat the offsprings as the parents of the next generation. The final stopping condition we choose is based on the iteration number.Here we want the 100th generation to be our final solution.

Following is a graph demonstrating how this select function works. 

\includegraphics[width=3.5in]{plot2.jpg}

parameter: Y ,dependent variable vector. Input should be a numeric vector

           X ,covariate matrix. Input should be a numeric data frame or matrix
           
           regType ,the type of regression, default is "lm". Input should be "lm" and "glm" 
           
           family ,the family of the glm, default is "gaussion". Input can be "binomial", "poisson",
           
           "Gamma","quasi", etc.
           
           fitness ,the method to evaluate the fitness of the model, default is AIC.
           
           rank ,indicates if we use rank as a selection criteria, default is TRUE. Input should 
           
           be logical
           
           selectionType, the selection criteria, default is "twoprop".
           
           elitism, indicates if there is a generation gap, default is TRUE. Iuput should be logical.
           
           crossoverType,  type of cutting points， defualt is "sinle". Input should be 
...
output: The individual with best fitness among the final generation. We constructed this object with four fields: variable, a 0-1 vector that indicates which features to choose; fit, the model associated with this individual; fitness, the fitness value of this model; type, what kind of fitness criteria is being used. 
